...
def chase_result(predator_mass, prey_mass, initial_separation):
...
def main():
    ...
    while(go_again == 1):
        ...
        separation = get_digit_input()
        print_text_by_key("guess", patron_type)
        guess_caught = get_digit_input() == 1

        # check catch
        result_dic = chase_result(predator_mass, prey_mass, separation)
        # show plot
        times = result_dic["times"]

        pylab.title("Chasing Model")
        pylab.xlabel("time(s)")
        pylab.ylabel("distance between predator and prey(m)")
        pylab.plot(times, result_dic["distances"], label='distances')
        pylab.plot(times, result_dic["predator_position"], label='predator')
        pylab.plot(times, result_dic["prey_position"], label='prey')
        pylab.legend()
        pylab.show()
        # guess result
        is_caught_prey = result_dic["is_caught_prey"]
